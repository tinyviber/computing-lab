...
mkdir -p "$DIST/assets" "$DIST/slides"
printf '<!doctype html><script type="module" src="/assets/app.js"></script>\n' >"$DIST/index.html"
printf 'console.log("fixture")\n' >"$DIST/assets/app.js"
printf 'PK pptx fixture\n' >"$DIST/slides/excel-01.pptx"
...
request_status /slides/excel-01.pptx
[[ "$HTTP_STATUS" == 200 ]] || fail "PPTX lesson returned $HTTP_STATUS"
grep -Fq 'PK pptx fixture' "$BODY_FILE" || fail 'PPTX lesson body was not served'

request_status /slides/unknown.pptx
[[ "$HTTP_STATUS" == 404 ]] || fail "unknown PPTX lesson returned $HTTP_STATUS"
pass 'Caddy serves the PPTX lesson and keeps unknown files at 404'
