...
预算不可编辑。页面同时显示 baseline、current、delta、采样像素数、变化像素数、平均误差和预算状态。delta 始终是 `current - baseline`；变化像素数沿用模型中“当前重建图与源图颜色不同的像素数”定义。

这里的位数和字节是理论原始像素数据量，不是 PNG、JPEG 或 WebP 的实际文件大小。格式实际大小仍取决于图像内容、编码器设置、文件头、元数据和具体实现。

预测只提供方向，不伪造未经计算的具体数值：

- 降低采样率：理论数据量通常下降，空间细节可能损失。
- 降低调色板位深：颜色数据量通常下降，色阶误差可能增加。
- 切换 RGB24 / 调色板：颜色精度和数据代价的取舍不同。
- 改变相位：采样位置变化，数据量不一定变化。
