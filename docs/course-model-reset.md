...
> **2026-09 修订（issue #35）**：本节“不需要 step / progress / submit”的结论已被
> 有意推翻。ALU 的关卡式模型在课堂上验证有效后，图像编码按“AI 修复老照片”主线
> 重做成分关课程（3 个客观判定的核心关 + 2 个选做 AI 挑战），见
> `docs/image-encoding-learning-flow.md`。下面的探索式分析仍作为领域模型参考保留；
> 其中 sampling/quantization/representation 的因果模型没有变，变化的是课程组织方式。
