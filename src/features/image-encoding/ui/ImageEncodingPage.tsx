...
function WorkflowRail({ step }: { step: number }) {
  return (
    <aside aria-label="Image encoding workflow" className="image-workflow">
...
function ImageEncodingContent({ search }: { search: Record<string, unknown> }) {
  ...
  return (
    <LabShell eyebrow="IMAGE / 01" title="图像编码" subtitle="Sampling and quantization">
      <div className="image-lab-layout">
        <WorkflowRail step={step} />
        <div className="image-feature-content">
          <section className="lesson-section image-visualization" aria-labelledby="preview-heading">
            ...
            <div className="summary-panel">
              ...
              <p className="metric-note">
                仅理论 pixel payload 比较；排除 palette、file
                header、metadata、container/codec、实际 file size/compression。
              </p>
            </div>
          </section>
          <aside className="image-inspector" aria-label="Encoding inspector">
            ...
            <FormulaPanel
              ...
              rows={[
                { label: "Raw baseline", value: "8×8 uncompressed 24-bit RGB source" },
                {
