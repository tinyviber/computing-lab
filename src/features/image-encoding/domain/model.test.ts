...
describe("image encoding domain model", () => {
  ...
  it("supports a fixed baseline budget of 25 percent of theoretical raw bits", () => {
    const baseline = deriveImageEncodingModel(source, {
      samplingPercent: 100,
      bitDepth: 24,
      colorMode: "rgb24",
      phase: 0,
    ...
    const budgetBits = Math.floor(baseline.rawPayload.bits * 0.25);

    expect(baseline.rawPayload.bits).toBe(240 * 160 * 24);
    expect(budgetBits).toBe(Math.floor((240 * 160 * 24 * 25) / 100));
    expect(budgetBits).toBeGreaterThan(0);
  });

  it("keeps model payload and error metrics finite at the largest supported settings", () => {
    const model = deriveImageEncodingModel(source, {
      samplingPercent: 100,
      bitDepth: 8,
      colorMode: "palette",
    ...
    for (const value of [
      model.rawPayload.bits,
      model.rawPayload.bytes,
      model.averageError,
      model.changedPixelCount,
    ]) {
